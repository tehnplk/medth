  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto">
          {hasDbError ? (
            <p className="mx-4 my-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">
              โหลดข้อมูลสาขาไม่สำเร็จ กรุณาตรวจสอบการเชื่อมต่อฐานข้อมูล
            </p>
          ) : null}

          {!hasDbError && branches.length === 0 ? (
            <p className="mx-4 my-3 rounded-xl border border-sky-200 bg-sky-50 px-3 py-2 text-sm text-sky-700">
              ยังไม่มีสาขาในระบบ
            </p>
          ) : null}
          ...
          {branches.map((branch) => (
            <Link
              ...
              className="flex items-start gap-4 bg-white px-4 py-4 transition active:bg-sky-50/70"
            >
